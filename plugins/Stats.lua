...
local function run(msg, matches)
  if matches[1]:lower() == 'redobot' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /redobot ")
    return about
  end 
  if matches[1]:lower() == "statslist" then
    if not is_momod(msg) then
      return "Das Nazan Bache :D"
    end
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "redobot" then -- Put everything you like :)
